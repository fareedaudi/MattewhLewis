...
const REACT_APP_DEPLOYMENT_URL = process.env.REACT_APP_DEPLOYMENT_URL;
...
class AppComponent extends React.Component {
  ...
  render() {
    console.log("Deployment URL:", REACT_APP_DEPLOYMENT_URL);
    let universities = this.props.data
      .filter(univ => univ.university_id <= 5)
      .sort((a, b) => a.university_id - b.university_id);
    let selectedUniversityId = this.state.selectedUniversityId;
    let university = universities.filter(
      univ => String(univ.university_id) === selectedUniversityId
    )[0];
    return (
      <div>
        <Header
          universities={universities}
          selectionHandler={this.onUniversitySelection}
        />

        {selectedUniversityId !== -1 ? (
          <Requirements
            university={university}
            coreRequirements={this.state.coreRequirements}
          />
        ) : (
          <HomePage />
        )}
