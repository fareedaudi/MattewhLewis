...
class SavedMapsContextProviderComponent extends React.Component {
  ...
  deleteMap = map_id => {
    var token = localStorage.getItem("PMTjwtToken");
    const Authorization = `Bearer ${token}`;
    axios
      .delete(`${ROOT_URL}/api/maps/${map_id}`, { headers: { Authorization } })
      .then(response => {
      ...
      .then(result => {
        this.setState({
          savedMaps: this.state.savedMaps.filter(
            savedMap => savedMap.id !== Number(map_id)
          )
        });
      });
  };
  ...
  getSavedMaps = () => {
    ...
    const Authorization = `Bearer ${token}`;
    return axios
      .get(`${ROOT_URL}/api/maps`, { headers: { Authorization } })
      .then(response => {
        if (response.status == 200) {
          return response.data;
        }
...
const SavedMapsContextProvider = WithLoginStatus(
  SavedMapsContextProviderComponent
);
    ...
    render() {
      return (
        <SavedMapsContext.Consumer>
          {savedMapsContext => (
            <SavedMapsConsumer
              savedMaps={savedMapsContext.savedMaps}
              {...this.props}
            />
          )}
        </SavedMapsContext.Consumer>
    ...
    render() {
      return (
        <SavedMapsContext.Consumer>
          {savedMapsContext => (
            <SavedMapsConsumer
              mapActionHandlers={savedMapsContext.mapActionHandlers}
              {...this.props}
            />
          )}
        </SavedMapsContext.Consumer>
