...
export default class LoginContextProvider extends React.Component {
  ...
  loadLoginData(token) {
    axios
      .post(`${ROOT_URL}/api/load_login_data`, { token })
  ...
  updateLoginData(loginDetails) {
    localStorage.setItem("PMTjwtToken", loginDetails.token);
    this.setState({
  ...
  loginFromCredentials({ loginEmail, loginPassword }) {
    return axios
      .post(`${ROOT_URL}/api/login`, { loginEmail, loginPassword })
      .then(response => {
...
const StayLoggedInModal = props => (
  ...
  >
    ...
    <ModalFooter>
      ...
      <Button color="primary" onClick={props.stayLoggedInHandler}>
        Yes, please
      </Button>
    </ModalFooter>
  </Modal>
    ...
    render() {
      return (
        <LoginContext.Consumer>
          {login => <LoginConsumer login={login} {...this.props} />}
        </LoginContext.Consumer>
      );
    }
  };
